...
export function setupFeedback(): void {
  configurationProperties.subscribe(configurations => {
    ...
    feedbackFormNotifications.update(store => {
      experimentalProperties.forEach(property => {
        const propertyID = property.id;
        if (!propertyID) return;
        let enabled = false;
        window
          .getConfigurationValue<boolean>(propertyID)
          .then(value => {
            enabled = typeof value === 'boolean' ? value : false;
          })
          .catch((err: unknown) => console.error(`Error getting configuration value ${propertyID}}`, err));
