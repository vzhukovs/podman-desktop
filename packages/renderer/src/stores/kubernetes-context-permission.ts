...
export const kubernetesContextsPermissions: Writable<ContextPermission[]> = writable([]);

// use helper here as window methods are initialized after the store in tests
const listContextsPermissions = (): Promise<ContextPermission[]> => {
  return window.kubernetesGetContextsPermissions();
};

export const kubernetesContextsPermissionsStore = new EventStore<ContextPermission[]>(
  'contexts permissions',
  kubernetesContextsPermissions,
  checkForUpdate,
  windowEvents,
  windowListeners,
  listContextsPermissions,
);
