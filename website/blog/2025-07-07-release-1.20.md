...
- **Add button to start all containers in bulk**: A new bulk-run button allows you to start multiple selected containers at once, saving time when launching your container stack.
- **Added switching users and clusters**: Seamlessly switch your active Kubernetes cluster and user context from within Podman Desktop, making multi-cluster workflows much easier.
- **Add search by description in extension list**: Find extensions faster by searching not just by name but also through keywords in their descriptions.
- **Allow provider update from resources page**: Easily update your container engines or Kubernetes providers right from the Resources page for a more streamlined upgrade process.
...
The container logs viewer can now be canceled mid-stream, allowing users to stop tailing logs when they are no longer needed.
Previously, once a container’s logs were opened, the output would continue streaming until the logs window was closed. With this update, an ongoing log stream can be interrupted via a cancel action without closing the logs pane, giving users more control over log monitoring. This improvement helps avoid redundant log output and unnecessary resource usage by letting log streaming be halted on demand.
...
- fix: remove full acces to d-bus and add missing --talk-name options by @dgolovin [#12778](https://github.com/podman-desktop/podman-desktop/pull/12778)
