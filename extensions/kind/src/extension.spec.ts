...
test('check we received notifications ', async () => {
  ...
  vi.mocked(podmanDesktopApi.provider.onDidRegisterContainerConnection).mockReturnValue({
    dispose: vi.fn(),
  } as extensionApi.Disposable);
  vi.mocked(podmanDesktopApi.provider.onDidUnregisterContainerConnection).mockReturnValue({
    dispose: vi.fn(),
  } as extensionApi.Disposable);
  ...
  onDidUpdateContainerConnectionMock.mockImplementation((callback: () => Promise<void>) => {
  ...
  extension.refreshKindClustersOnProviderConnectionUpdate(PROVIDER_MOCK);
