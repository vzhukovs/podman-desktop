...
test.describe
  .serial('Managed Configuration - preferences', { tag: '@managed-configuration' }, () => {
    test.describe
    ...
    test.describe
      .serial('Locked preference: Line Height', () => {
        ...
        test('Preference can not be changed', async () => {
          const preferenceInput = lineHeightRow.locator(
            `input[name="${Preferences.TERMINAL_LINE_HEIGHT_INPUT_LABEL}"]`,
          );
